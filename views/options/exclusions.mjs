...
function main () {
  const exclusionsContainer = document.getElementById('exclusionsContainer');
        exclusionsContainer.dataset.noEntriesHint = chrome.i18n.getMessage('exclusionsHintNoEntries');
  const exclusionsForm = document.getElementById('exclusionsForm');
        exclusionsForm.onsubmit = onFormSubmit;
        exclusionsForm.elements.urlPattern.placeholder = chrome.i18n.getMessage('exclusionsPlaceholderURL');
        exclusionsForm.elements.urlPattern.title = chrome.i18n.getMessage('exclusionsPlaceholderURL');
...
function onInputChange () {
  if (Config.get("Exclusions").indexOf(this.value.trim()) !== -1) {
    this.setCustomValidity(chrome.i18n.getMessage('exclusionsNotificationAlreadyExists'));
  }
