...
export const Config = new ConfigManager("local", chrome.runtime.getURL("/resources/json/defaults.json"));
...
for (let element of document.querySelectorAll('[data-include]')) {
  const fetchingHTML = fetchHTMLAsFragment(chrome.runtime.getURL(element.dataset.include));
...
function onPageNavigation () {
  ...
  if (nextItem) {
    ...
    document.title = `Gesturefy - ${chrome.i18n.getMessage(sectionKey)}`;
  }
}
