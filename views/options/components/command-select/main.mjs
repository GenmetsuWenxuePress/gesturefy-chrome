...
const COMMAND_ITEMS = fetchJSONAsObject(chrome.runtime.getURL("/resources/json/commands.json"));
const COMMAND_SETTING_TEMPLATES = fetchHTMLAsFragment(chrome.runtime.getURL("/views/options/fragments/command-setting-templates.inc"));
...
class CommandSelect extends HTMLElement {
  ...
  attributeChangedCallback (name, oldValue, newValue) {
    if (name === "value") {
      ...
      if (this.command instanceof Command) {
        ...
        secondaryAccessButton.title = chrome.i18n.getMessage("commandBarOpenSelectionText");
      }
    }
  }
  ...
  async _buildCommandsPanel () {
    ...
    const commandsHeading = templateFragment.getElementById("commandsHeading");
          commandsHeading.title = commandsHeading.textContent = chrome.i18n.getMessage('commandBarTitle');
    ...
    const commandsSearchInput = templateFragment.getElementById('commandsSearchInput');
          ...
          commandsSearchInput.placeholder = chrome.i18n.getMessage('commandBarSearch');
    ...
    for (let commandItem of await COMMAND_ITEMS) {
      ...
      const description = document.createElement("span");
            ...
            description.textContent = chrome.i18n.getMessage(`commandDescription${commandItem.command}`);
  ...
  async _buildSettingsPanel () {
    ...
    const saveButton = document.createElement("button");
          ...
          saveButton.textContent = chrome.i18n.getMessage('buttonSave');
