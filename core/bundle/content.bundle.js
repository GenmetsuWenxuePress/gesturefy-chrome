  ...
  remove (storagePath) {
    ...
    if (storagePath.length > 0) {
      ...
      if (storagePath.length === 1) {
        return chrome.storage[this._storageArea].remove(storagePath[0]);
      }
...
const PopupURL = new URL(chrome.runtime.getURL("/core/views/popup-command-view/popup-command-view.html"));
...
// The conversation to css screen coordinates via 0 is required
